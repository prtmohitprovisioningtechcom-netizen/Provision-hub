...
export function Footer({ config }: { config?: any }) {
  ...
  const copyrightText = footerConfig.copyrightText || `© ${new Date().getFullYear()} ${siteConfig.name}. All rights reserved.`;
  ...
  return (
    <footer className="border-t border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid gap-8 md:grid-cols-4">
          <div>
            ...
            <div className="flex items-center gap-4 mt-6">
              {footerConfig.facebookLink && footerConfig.facebookLink !== '#' && (
                <a href={footerConfig.facebookLink} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-indigo-600 transition-colors">
              ...
              {footerConfig.twitterLink && footerConfig.twitterLink !== '#' && (
                <a href={footerConfig.twitterLink} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-indigo-600 transition-colors">
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" /></svg>
                </a>
              )}
              {footerConfig.instagramLink && footerConfig.instagramLink !== '#' && (
                <a href={footerConfig.instagramLink} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-indigo-600 transition-colors">
              ...
              {config?.linkedinLink && config.linkedinLink !== '#' && (
                <a href={config.linkedinLink} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-indigo-600 transition-colors">
        ...
        <div className="mt-12 border-t border-gray-200 dark:border-gray-800 pt-8 text-center text-sm text-gray-500">
          {copyrightText}
        </div>
      </div>
    </footer>
