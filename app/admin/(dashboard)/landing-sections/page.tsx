...
export default function AdminLandingSections() {
  ...
  const fetchCategories = async () => {
    try {
      const { data } = await axios.get('/api/admin/landing-sections');
      if (data.success) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { data } = await axios.delete(`/api/admin/landing-sections/${id}`);
      if (data.success) {
        toast.success('Landing section deleted');
        fetchCategories();
      }
    } catch (error: any) {
      toast.error(error.response?.data?.error || 'Failed to delete landing section');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && currentId) {
        const { data } = await axios.put(`/api/admin/landing-sections/${currentId}`, formData);
        if (data.success) {
        ...
        const { data } = await axios.post('/api/admin/landing-sections', formData);
        if (data.success) {
      ...
      toast.error(error.response?.data?.error || 'Failed to save landing section');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Landing Sections</h1>
        <p className="mt-2 text-gray-500">Manage available sections for the Website Builder.</p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1 h-fit">
          <CardHeader>
            ...
            <CardDescription>
              {isEditing ? 'Update the details below.' : 'Add a new section template for the Website Builder.'}
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>All Sections ({categories.length})</CardTitle>
          </CardHeader>
          <CardContent>
